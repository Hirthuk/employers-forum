...
const Appreciate = () => {
  ...
  const [reason, setReason] = useState(''); // new dropdown state
  ...
  // Track how many appreciations current user has sent (monthly)
  const [sentCount, setSentCount] = useState(0);
  const MAX_SENT = 2;

  const getPostDate = (post) => {
    const dateStr = post.creation_date ?? post.createdAt ?? post.creationDate ?? post.created_at ?? post.date;
  ...
  // Fetch count of appreciations sent by current user in current month
  useEffect(() => {
    let mounted = true;
    const fetchSentCount = async () => {
      try {
        ...
        if (!mySap) {
          if (mounted) setSentCount(0);
          return;
        }
        const resp = await axiosApiClient.get(API_CONFIG.EndPoints.GETHAPPENINGS);
        const posts = resp?.data || [];
        const now = new Date();
        const count = posts.filter(p => {
          const from = String(p.from_sapid ?? p.fromSapId ?? p.from ?? '');
          if (from !== String(mySap)) return false;
          const d = getPostDate(p);
          return isSameMonth(d, now);
        }).length;
        if (mounted) setSentCount(count);
      } catch (e) {
        console.error('Failed to fetch sent appreciations count', e);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!reason) {
      toast.error('Please select a reason for appreciation', {
        position: "top-center",
        autoClose: 3000,
      });
      return;
    ...
    try {
      ...
      // Prepare the request data (include the selected reason/header)
      const requestData = {
      ...
      // refresh sent count after successful send (recalculate for current month)
      try {
        const resp = await axiosApiClient.get(API_CONFIG.EndPoints.GETHAPPENINGS);
        const posts = resp?.data || [];
        const now = new Date();
        const newCount = posts.filter(p => {
          const from = String(p.from_sapid ?? p.fromSapId ?? p.from ?? '');
          if (from !== String(fromSapId)) return false;
          const d = getPostDate(p);
          return isSameMonth(d, now);
        }).length;
        setSentCount(newCount);
      } catch (e) {
        console.error('Failed to refresh sent count', e);
        setSentCount(prev => Math.min(prev + 1, MAX_SENT));
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-indigo-100">
      ...
      <div className="max-w-md mx-auto px-4 py-10 sm:py-16">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl relative">
          {/* Decorative elements */}
          ...
          <div className="p-6 sm:p-8 relative z-10">
            <form onSubmit={handleSubmit} className="space-y-6">
              {sentCount >= MAX_SENT && (
                <div className="p-3 mb-2 rounded-md bg-yellow-50 border border-yellow-100 text-yellow-800 text-sm">
              ...
              {/* Appreciation Header / Reason Dropdown */}
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="Thanks for help buddy">Thanks for help buddy</option>
                    <option value="Good performance in an event">Good performance in an event</option>
                    <option value="Great leadership">Great leadership</option>

                    {/* Additional options */}
                    <option value="Outstanding teamwork">Outstanding teamwork</option>
                    <option value="Innovative solution">Innovative solution</option>
                    <option value="Excellent customer service">Excellent customer service</option>
                    <option value="Mentorship and guidance">Mentorship and guidance</option>
                    <option value="Consistent reliability">Consistent reliability</option>
                    <option value="Going above and beyond">Going above and beyond</option>
                    <option value="Positive attitude">Positive attitude</option>
                    <option value="Process improvement">Process improvement</option>
                    <option value="Timely delivery">Timely delivery</option>
                    <option value="High quality work">High quality work</option>
                  </select>
                </div>
              </div>
